...
def getSkewAngle(img) -> float:
    ...
    # find all contours
    cnts, hierarchy = cv2.findContours(dilate, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # find largest contour and surround with min area box
...
# Deskew image
def deskew_v2(cvImage):
    angle = getSkewAngle(cvImage)
    return rotateImage(cvImage, -1.0 * angle)

def remove_border_straight(img):
    # convert to grayscale
    image_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    image_thresh = cv2.threshold(image_gray, 90, 255, cv2.THRESH_BINARY)[1]
    # find all contours
    cont, hier = cv2.findContours(image_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for c in cont:
        ...
        if area > 5000:
            cv2.drawContours(image_gray, [c], -1, (255, 255, 255), 20) # draw filled contour on mask
    masked = cv2.bitwise_not(mask)
    image_mask = cv2.bitwise_or(image_gray, masked)
...
def ocr_img(
    ...
    if input_file:
        ...
        img = remove_border_straight(img)
    ...
    details = pytesseract.image_to_data(
        bin_img, output_type=Output.DICT, config=config_param, lang='vie')
    ...
    if generate_output and details:
        ...
        """IF ONLY HAS A IMAGE FILE, SAVE THE OUTPUT TO A TEXT FILE"""
        # pdfContent = pd.DataFrame(columns=['page', 'line_id', 'line'])
        # pdfContent = save_page_content(pdfContent = pdfContent, page_id = 0, page_data=output_data)
...
def ocr_file(**kwargs):
    ...
    for pg in range(pdfIn.page_count):
        ...
        # Get Page Size
        pageo = pdfOut.new_page(
            pno=-1, width=page.rect.width, height=page.rect.height)
        pageo.insert_image(page.rect, stream=upd_array)
    content_file = None
    if generate_output:
        content_file = save_file_content(
            pdfContent=pdfContent, input_file=input_file)
    summary = {
        "File": input_file, "Total pages": pdfIn.page_count, 
